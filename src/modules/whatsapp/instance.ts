...
export class WhatsAppInstance {
    ...
    async init() {
        ...
        this.socket.ev.on("groups.upsert", async (groups) => {
            try {
                const sessionDb = await prisma.session.findUnique({ where: { sessionId: this.sessionId }, select: { id: true } });
                if (!sessionDb) return;
                
                for (const g of groups) {
                    await prisma.group.upsert({
                        where: { sessionId_jid: { sessionId: sessionDb.id, jid: g.id } },
                        create: {
                            sessionId: sessionDb.id, jid: g.id, subject: g.subject, description: g.desc,
                            ownerJid: g.owner, creation: g.creation ? new Date(g.creation * 1000) : undefined,
                            restrict: g.restrict, announce: g.announce, participants: g.participants as any, metadata: g as any
                        },
                        update: {
                            subject: g.subject, description: g.desc, ownerJid: g.owner,
                            restrict: g.restrict, announce: g.announce, participants: g.participants as any, metadata: g as any
                        }
                    });
                }
                    ...
                    if (Object.keys(updateData).length > 0) {
                        try {
                            await prisma.group.update({
                                where: { sessionId_jid: { sessionId: sessionDb.id, jid: u.id } },
                                data: updateData
                            });
                        } catch (e) {

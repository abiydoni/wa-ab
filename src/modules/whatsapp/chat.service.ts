...
export class ChatService {
    ...
    static async getChatsList(dbSessionId: string) {
        ...
        const allJids = new Set([
            ...contacts.map((c: any) => c.jid),
            ...groups.map((g: any) => g.jid),
            ...messagesWithDistinctJids.map((m: any) => m.remoteJid)
        ]);
        ...
        const groupMap = new Map(groups.map((g: any) => [g.jid, g]));
        ...
        const chatList = await Promise.all(Array.from(allJids).map(async (originalJid) => {
            ...
            let contactInfo: any = { jid: normalizedJid, name: null, notify: null, profilePic: null };
            if (contactMap.has(originalJid) || contactMap.has(normalizedJid)) {
                ...
                const g: any = groupMap.get(originalJid) || groupMap.get(normalizedJid);
                contactInfo = { jid: g?.jid, name: g?.subject, notify: null, profilePic: null };
            }
